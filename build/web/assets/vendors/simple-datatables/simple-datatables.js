          ...
          class DataTable {
            ...
            render(t) {
              ...
              if (
                ...
                (s += "</div>"),
                e.scrollY.length
                  ? (s += `<div class='dataTable-container' style='height: ${e.scrollY}; overflow-Y: auto;margin-bottom: 16px;'></div>`)
                  : (s += "<div class='dataTable-container' style='margin-bottom: 16px;'></div>"),
                (s +=
                  "<div class='dataTable-bottom' style='position: relative;'>"),
                (s += e.layout.bottom),
                (s = (s += "</div>").replace(
                  "{info}",
                  e.paging
                    ? "<div class='dataTable-info' style='position: relative;top: 12px;'></div>"
                    : ""
                )),
                e.paging && e.perPageSelect)
              ) {
                let t = "<div class='dataTable-dropdown' style='margin-top: -22px;'>";
                (t += e.labels.perPage), (t += "<label></label></div>");
                const a = createElement("select", {
                  class: "dataTable-selector form-select",
                  style: "position: absolute;bottom: 18px;z-index: 200;right:208px"
                });
