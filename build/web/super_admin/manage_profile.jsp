...
<%@ page import="com.dao.SaDao" %>
<%@ page import="com.entities.SuperAdmin,com.entities.Designation,com.helper.ConnectionProvider" %>
...
<%
    SaDao saDao = new SaDao(ConnectionProvider.getConnection());
    SuperAdmin sa = saDao.getSa(user.getId());
//    out.println(sa.getSa_image().toString());
//out.println(sa);
    
    byte[] imageData = sa.getSa_image();
    
    String base64Image = java.util.Base64.getEncoder().encodeToString(imageData);

    java.sql.Timestamp createdAccountTimestamp = sa.getCreatedAccount();
    ...
    java.util.Date DateOfBirth = sa.getDob();
                ...
                <div class="card mb-3" style="border-radius: .5rem;">
                    <div class="row g-0">
                        <div class="col-md-4 gradient-custom text-center text-white"
                            ...
                            <h5><%= sa.getFname() +" "+ sa.getLname() %></h5>
                            
                            <a style="cursor: pointer;color: white" name="<%= sa.getId()%>" class="editPersonal"
                               data-image-src="data:image/png;base64,<%= base64Image%>" data-fname="<%= sa.getFname()%>"
                               data-lname="<%= sa.getLname()%>" data-email="<%= sa.getEmail()%>"
                               data-gender="<%= sa.getGender()%>" data-address="<%= sa.getAddress()%>"
                               data-createdaccount="<%= createdAccountDateStr%>" 
                               data-contact="<%= sa.getContact()%>" 
                               data-useridhr="<%= sa.getUser_id_sa()%>"
                            ...
                            <div class="card-body p-4">
                                ...
                                <div class="row pt-1">
                                    <div class="col-6 mb-3">
                                        <h6>Email</h6>
                                        <p class="text-muted"><%= sa.getEmail()%></p>
                                    </div>
                                    <div class="col-6 mb-3">
                                        <h6>Phone</h6>
                                        <p class="text-muted"><%= sa.getContact()%></p>
                                    </div>
                                    <div class="col-6 mb-3">
                                        <h6>Birthday</h6>
                                    ...
                                    <div class="col-6 mb-3">
                                        <h6>Gender</h6>
                                        <p class="text-muted"><%= sa.getGender()%></p>
                                    </div>
                                    <div class="col-6 mb-6">
                                        <h6>Address</h6>
                                        <p class="text-muted"><%= sa.getAddress()%></p>
                                    </div>
                                    <div class="col-6 mb-6">
                                        <h6>Joining Date</h6>
